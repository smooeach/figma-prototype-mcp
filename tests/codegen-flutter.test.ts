...
describe("emitFlutter", () => {
  ...
  it("router.dart has a Screen enum value per frame + Router class", () => {
  ...
  it("Actions file maps navigate/openUrl/conditional/back", () => {
    ...
    expect(a).toContain("void goDetail(Router router, PrototypeStore store)");
...
describe("emitFlutter source without a name", () => {
  ...
  it("derives a valid Dart function name from a digit-leading node id (no name)", () => {
    ...
    expect(a).toContain("void n1_2(Router router, PrototypeStore store)");
