...
export function emitStore(spec: InteractionSpec): string {
  ...
  return [
    ...
    `  overlay: { screen: string; style: "sheet" | "dialog"; dismissable: boolean } | null;`,
    `  presentOverlay: (o: { screen: string; style: "sheet" | "dialog"; dismissable: boolean }) => void;`,
    `  dismissOverlay: () => void;`,
    `} | null>(null);`,
    ``,
    ...
    `  const [overlay, setOverlay] = useState<{ screen: string; style: "sheet" | "dialog"; dismissable: boolean } | null>(null);`,
    `  const presentOverlay = useCallback(function presentOverlay(o: { screen: string; style: "sheet" | "dialog"; dismissable: boolean }) { setOverlay(o); }, []);`,
    `  const dismissOverlay = useCallback(function dismissOverlay() { setOverlay(null); }, []);`,
    `  return <StoreContext.Provider value={{ vars, set, toggle, overlay, presentOverlay, dismissOverlay }}>{children}</StoreContext.Provider>;`,
    `}`,
    ``,
