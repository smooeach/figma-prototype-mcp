...
export const GenerateInteractionCodeInput = z
  .object({
    ...
    target: z
      ...
      .describe("Target framework. react (react-router + framer-motion) or react-native (@react-navigation)."),
    pageId: z.string().optional(),
  })
  .strict()
  .describe(
    "Generate framework code from a prototype's wired interactions (navigation, transitions, variables, " +
      "conditionals) — NOT screen UI. Pick `target` ('react' or 'react-native'). Returns a structured set of files " +
