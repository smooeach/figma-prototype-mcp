...
export const GenerateInteractionCodeInput = z
  .object({
    ...
    target: z
      .enum(["react", "react-native", "swiftui", "compose", "flutter"])
      .describe("Target framework. react (react-router+framer-motion), react-native (@react-navigation), swiftui (NavigationStack), compose (Navigation Compose), or flutter (GoRouter)."),
    pageId: z.string().optional(),
  })
  .strict()
  .describe(
    "Generate framework code from a prototype's wired interactions (navigation, transitions, variables, " +
      "conditionals) — NOT screen UI. Pick `target` ('react', 'react-native', 'swiftui', 'compose', or 'flutter'). Returns a structured set of files " +
